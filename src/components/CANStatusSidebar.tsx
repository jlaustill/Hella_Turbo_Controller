    ...
    const initializeCAN = async () => {
      if (!canServiceRef.current) {
        ...
        const messageListener = (serviceMessage: ServiceCANMessage) => {
          ...
          setLastMessage(
            `0x${serviceMessage.id.toString(16).toUpperCase()}: ${Array.from(
              serviceMessage.data,
            )
      ...
      <CardContent sx={{ flexGrow: 1, pb: 1 }}>
        ...
        {connectionStatus === "disconnected" && (
          <Typography variant="body2" color="text.secondary" sx={{ mt: 2 }}>
            Configure your CAN interface above, then connect to begin monitoring
            Hella turbo actuator traffic.
          </Typography>
        )}
      </CardContent>
