 ...
 */
async function setupCANInterface(channel: string, bitrate: number): Promise<{ success: boolean; message: string }> {
...
async function setupRealCANSocket(channel: string): Promise<SocketCANChannel> {
  ...
  console.log(`Setting up real CAN socket for ${channel} using socketcan library`);
  return socket;
}
...
if (require.main === module) {
    ...
    try {
      ...
      if (!connection) {
        ...
        connection.socket.addListener('onMessage', (msg: any) => {
          ...
          console.log(`📡 Real CAN message received: ID=0x${msg.id.toString(16)}, Data=[${Array.from(msg.data).map((b: number) => '0x' + b.toString(16).padStart(2, '0')).join(', ')}]`);
  ...
  server.listen(PORT, () => {
    console.log(`🚀 CAN API server with WebSocket support running on port ${PORT}`);
    console.log(`📡 Using mature Node.js socketcan library v${require('../../../node_modules/socketcan/package.json').version}`);
